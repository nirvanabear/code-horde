...
Plan: 			Perform the steps, write them out, describe the process
    ...
    create dictionary with letters and values
    reverse string
    store each char
        compare to next char
        if equal or smaller, add value to aggregate
        if larger,  subtract next char from current, add to aggregate
            and skip counter past next char,
                if end of string, end loop
        if end of string, add value to aggregate
